# DATA MANAGER INSTITUCIONAL ELITE V16 - TRANSFER & EMAIL
# 1. Corrige Diluição (Sliding Window)
# 2. Captura Total (ID Discovery)
# 3. Link de Download no Log (Transfer.sh)
...
def upload_to_transfer(file_path):
    print(f"\n[TRANSFER] Subindo arquivo para link de download...")
    try:
        with open(file_path, 'rb') as f:
            response = requests.put(f'https://transfer.sh/{os.path.basename(file_path)}', data=f)
        if response.status_code == 200:
            print("\n" + "="*50)
            print(f"   LINK DE DOWNLOAD: {response.text.strip()}")
            print("="*50 + "\n")
            return response.text.strip()
    except Exception as e:
        print(f"[ERRO TRANSFER] {e}")
    return None

def send_email(file_path, target_email):
    print(f"[EMAIL] Preparando envio para {target_email}...")
    # NOTA: Para funcionar, você precisaria configurar um servidor SMTP.
    # Como link de download é mais garantido no Render, vamos focar nele.
    pass
...
def main():
    ...
    # Entrega via Link (Funciona 100% no Render)
    upload_to_transfer(zip_name)
