...
# ============================================================
# 🔒 DETERMINISMO TOTAL - SEEDS FIXOS
# ============================================================
# Garante resultados reproduzíveis entre execuções
SEED = 42
...
print(f"   Seed fixo: {SEED}")
print(f"   random, numpy, hash → fixados")
print(f"   n_jobs=1 (single-thread) → modelos determinísticos")
print("=" * 80)
print()
...
def treinar_um_target(target_col, df, outdir):
    ...
    try:
        model_lgb = LGBMClassifier(
            ...
            learning_rate=0.03,
            max_depth=-1,
            n_jobs=1,  # 🔒 DETERMINISMO: single-thread
            random_state=42,  # 🔒 DETERMINISMO: seed fixo
...
# ============================================================================
# DEBUG - VERIFICAR DISCO PERSISTENTE
# ============================================================================
print("\n" + "=" * 80)
print("🔍 DEBUG - VERIFICANDO DISCO PERSISTENTE")
print("=" * 80)
...
# Possíveis localizações do disco persistente no Render
possiveis_paths = [
...
for path in possiveis_paths:
    ...
    print(f"{simbolo} {path}")
    
    if existe:
        # Listar conteúdo
        try:
            conteudo = os.listdir(path)
            print(f"   └─ Conteúdo ({len(conteudo)} itens):")
            for item in sorted(conteudo)[:15]:  # Primeiros 15
                ...
                if os.path.isdir(item_path):
                    ...
                    try:
                        ...
                        item_name = f"{item}/ ({sub_count} itens)"
                    except:
                        item_name = f"{item}/"
                else:
                    tipo = "📄"
                    try:
                        ...
                        item_name = f"{item} ({size_str})"
                    except:
                        item_name = item
                
                print(f"      {tipo} {item_name}")
            
            if len(conteudo) > 15:
                print(f"      ... (+{len(conteudo)-15} itens)")
        except Exception as e:
...
render_vars = {
    'RENDER': os.environ.get('RENDER', 'N/A'),
    'RENDER_SERVICE_NAME': os.environ.get('RENDER_SERVICE_NAME', 'N/A'),
    'RENDER_EXTERNAL_URL': os.environ.get('RENDER_EXTERNAL_URL', 'N/A'),
    'RENDER_PERSISTENT_DISK_MOUNT_PATH': os.environ.get('RENDER_PERSISTENT_DISK_MOUNT_PATH', 'N/A'),
    'RENDER_GIT_COMMIT': os.environ.get('RENDER_GIT_COMMIT', 'N/A'),
}

for var, valor in render_vars.items():
    print(f"  {var}: {valor}")
...
# Verificar OUT_DIR usado no código
print("\n📂 OUT_DIR USADO NO CÓDIGO:")
try:
    print(f"  {OUT_DIR}")
    if os.path.exists(OUT_DIR):
        print(f"  ✅ Existe!")
        try:
            conteudo = os.listdir(OUT_DIR)
            print(f"  └─ {len(conteudo)} itens dentro")
        except:
            pass
    else:
        print(f"  ❌ NÃO EXISTE!")
except:
    print(f"  ⚠️  Variável OUT_DIR não definida")
...
if IS_RENDER:
    ...
    BASE_PATH = os.environ.get('RENDER_PERSISTENT_DISK_MOUNT_PATH')
    
    # Prioridade 2: Caminho padrão
    if not BASE_PATH or not os.path.exists(BASE_PATH):
        BASE_PATH = "/opt/render/project/.data"
    
    # Prioridade 3: OUT_DIR se existe
    ...
    BASE_PATH = os.getcwd()
    print(f"⚠️  AMBIENTE LOCAL DETECTADO")
...
if not all_files:
    ...
    print()
    print("💡 DICA: Os arquivos podem estar em outro diretório!")
    print("   Verifique o output do DEBUG acima.")
else:
    # Ordenar por caminho
    ...
    if pkls:
        ...
        if lgbm:
            ...
            for f in sorted(lgbm, key=lambda x: x['path']):
                print(f"  {f['size_str']:>12} | {f['modified']} | {f['path']}")
        
        if xgb:
            print(f"\n🌳 XGB ({len(xgb)} modelos):")
            for f in sorted(xgb, key=lambda x: x['path']):
                print(f"  {f['size_str']:>12} | {f['modified']} | {f['path']}")
        
        if outros_pkl:
            print(f"\n🔧 Outros PKLs ({len(outros_pkl)}):")
            for f in sorted(outros_pkl, key=lambda x: x['path']):
                print(f"  {f['size_str']:>12} | {f['modified']} | {f['path']}")
    ...
    if outros:
        ...
        print("=" * 80)
        for f in outros[:20]:  # Limitar a 20
            print(f"{f['size_str']:>12} | {f['modified']} | {f['path']}")
        if len(outros) > 20:
            print(f"... (+{len(outros)-20} arquivos)")
    ...
    # Arquivos esperados
    esperados_csv = ['15m.csv', '30m.csv', '1h.csv', '4h.csv', '8h.csv', '1d.csv']
    esperados_pkl = ['scaler_regimes.pkl', 'kmeans_regimes.pkl']
    
    problemas = []
    
    # Verificar CSVs
    csv_names = [f['path'] for f in csvs]
    for esperado in esperados_csv:
    ...
    num_modelos = len([f for f in pkls if any(x in f['path'].lower() for x in ['lgbm', 'xgb', 'target_'])])
    if num_modelos >= 13:
        print(f"✅ Modelos: {num_modelos} modelos encontrados (esperado ≥13)")
    else:
        print(f"⚠️  Modelos: {num_modelos} modelos encontrados (esperado ≥13)")
        problemas.append(f"Faltam modelos (tem {num_modelos}, esperado ≥13)")
