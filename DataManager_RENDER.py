# DATA MANAGER INSTITUCIONAL ELITE V16 - RENDER VERSION
# Arquitetura de Agregação Inteligente (1m -> Multi-TF)
# Otimizado para Microestrutura e Alta Performance
# REMOVIDOS INPUTS PARA RODAR EM BACKGROUND WORKER
...
def download_agg_trades_parallel(symbol, start_ms, end_ms, min_val_usd=100):
    print(f"[INFO] Baixando AggTrades para {symbol}...")
    all_chunks = []
    total_trades = 0
    hour_ms = 3600 * 1000
    curr = start_ms
    with requests.Session() as session:
        while curr < end_ms:
            try:
                ...
                while True:
                    time.sleep(0.1) 
                    url = f"https://fapi.binance.com/fapi/v1/aggTrades?symbol={symbol}&startTime={curr}&endTime={next_stop}&limit=1000" if last_id is None else f"https://fapi.binance.com/fapi/v1/aggTrades?symbol={symbol}&fromId={last_id + 1}&limit=1000"
                    ...
                    if int(res_raw[-1]['T']) >= next_stop or len(res_raw) < 1000: break
...
def main():
    ...
    simbolo = dm_cfg.get('symbol', 'PENDLEUSDT')
    tf = dm_cfg.get('timeframe', '15m')
    days_back = dm_cfg.get('days_back', 180)
    min_val = dm_cfg.get('min_val_usd', 500)
    output_file = dm_cfg.get('output_path', f"/data/{simbolo}_{tf}.csv")
    end_dt = datetime.now()
    start_dt = end_dt - timedelta(days=days_back)
    start_ms = int(start_dt.timestamp() * 1000)
    end_ms = int(end_dt.timestamp() * 1000)
    print(f"Configuração: {simbolo} | {tf} | {days_back} dias | Min: ${min_val}")
    pasta_saida = os.path.dirname(output_file)
    if pasta_saida and not os.path.exists(pasta_saida): os.makedirs(pasta_saida)
    ...
    if trades_np is not None: df_final = processar_microestrutura_dinamica(df_final, trades_np, tf)
