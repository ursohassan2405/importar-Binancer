# DATA MANAGER INSTITUCIONAL ELITE V16 - V3 INTEGRATED (AUTO-GITHUB)
# Gera os dados, compacta e envia para o GitHub automaticamente.
...
# --- CONFIGURAÇÕES DE DOWNLOAD (V3 ID DISCOVERY) ---
...
def upload_to_github(file_path):
    print(f"\n[GITHUB] Iniciando processo de upload para {file_path}...")
    
    # 1. Compactar
    zip_name = file_path.replace(".csv", ".zip")
    with zipfile.ZipFile(zip_name, 'w', zipfile.ZIP_DEFLATED) as zipf:
        zipf.write(file_path, os.path.basename(file_path))
    print(f"[GITHUB] Arquivo compactado: {zip_name}")

    # 2. Configurações do Git (Usando variáveis de ambiente do Render)
    # Se não houver variáveis, o script falhará com aviso.
    token = os.getenv("GITHUB_TOKEN")
    repo = os.getenv("GITHUB_REPO") # Formato: usuario/repositorio
    user_email = os.getenv("GITHUB_EMAIL", "render-bot@manus.im")
    user_name = os.getenv("GITHUB_NAME", "Render Bot")

    if not token or not repo:
        print("[ERRO GITHUB] GITHUB_TOKEN ou GITHUB_REPO não configurados no Render!")
        return
    ...
    try:
        ...
        # Inicializa se necessário ou apenas adiciona
        ...
        subprocess.run(["git", "commit", "-m", f"Update data: {os.path.basename(zip_name)}"], check=True)
        subprocess.run(["git", "push", "-u", "origin", "main", "--force"], check=True)
        
        print(f"[SUCESSO GITHUB] Arquivo {zip_name} enviado para o repositório!")
    except Exception as e:
        print(f"[ERRO GITHUB] Falha no push: {e}")
...
def main():
    ...
    print(f"\n[SUCESSO] Arquivo gerado localmente: {saida_csv}")
