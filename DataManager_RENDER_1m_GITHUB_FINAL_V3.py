...
# LÊ O TOKEN DA VARIÁVEL DE AMBIENTE (SOLUÇÃO CORRETA)
...
def upload_file_to_github(token, repo_full_name, file_path):
    ...
    if not token:
        print("[ERRO GITHUB] Token de acesso não encontrado. Configure a variável de ambiente GITHUB_TOKEN.")
        return None

    print(f"[GITHUB] Tentando fazer upload de {os.path.basename(file_path)} para {repo_full_name}")
    
    response_put = None # Inicializa a variável
    ...
    if response_get.status_code == 200:
        ...
        print(f"[GITHUB] Arquivo não encontrado. Criando novo.")
    else:
        print(f"[ERRO GITHUB] Falha ao verificar arquivo ({response_get.status_code}): {response_get.json().get('message', 'Erro desconhecido')}")
        return None
    ...
    if response_put is not None and response_put.status_code in [200, 201]:
