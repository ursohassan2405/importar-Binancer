# DATA MANAGER INSTITUCIONAL ELITE V16 - MANUS UPLOAD EDITION
# 1. Corrige Diluição (Sliding Window)
# 2. Captura Total (ID Discovery)
# 3. Upload Direto para Manus
...
def download_agg_trades_parallel(symbol, start_ms, end_ms, min_val_usd=500):
    print(f"[INFO] Iniciando Captura de Baleias...")
    ...
    with requests.Session() as session:
        while True:
            try:
                ...
                if last_id % 10000 < 1000:
                    print(f"    >>> Progresso: {pd.to_datetime(last_ts, unit='ms')} | Baleias: {total_whales}")
...
def download_futures_klines(symbol, interval, start_ms, end_ms):
    print(f"[INFO] Baixando Klines 1m...")
...
def processar_microestrutura_dinamica(df_klines_1m, trades_np, tf_str):
    print(f"[INFO] Consolidando para {tf_str}...")
...
def main():
    ...
    print(f"[MANUS] O arquivo está pronto para ser coletado.")
