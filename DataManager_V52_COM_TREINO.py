...
# 4. Parâmetros fixos: candles=5, multiframe, peso_temporal=1
# ============================================================
...
def adicionar_features_avancadas(df: pd.DataFrame) -> pd.DataFrame:
    """Features Avançadas - CÓPIA EXATA DO V27."""
    df = df.copy()
    ...
    if "aggression_delta" in df.columns:
        df["flow_dominance"] = df["aggression_delta"]
        df["flow_acceleration"] = df["aggression_delta"].diff()

    # 17. DISTÂNCIAS NORMALIZADAS
...
def treinar_modelo_v27(df_15m, out_dir=OUT_DIR):
    ...
    """
    print("\n" + "="*70, flush=True)
    print("⭐ INICIANDO TREINO V27", flush=True)
    print("="*70, flush=True)
