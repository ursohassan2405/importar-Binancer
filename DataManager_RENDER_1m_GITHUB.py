#!/usr/bin/env python3
"""
DATA MANAGER RENDER 1M - Otimizado para Download Rápido, Cronometragem e Upload Único via GitHub
"""
...
def create_github_repo(token, repo_name):
    """Cria um novo repositório privado no GitHub."""
    print(f"[GITHUB] Tentando criar repositório: {repo_name}")
    ...
    data = {
        "name": repo_name,
        "description": "Dados de backtest temporários (1m) - Excluir após download.",
        "private": True,
        "auto_init": False
    }
    ...
    if response.status_code == 201:
        print("[GITHUB] Repositório criado com sucesso.")
        return response.json()['full_name']
    else:
        print(f"[ERRO GITHUB] Falha ao criar repositório ({response.status_code}): {response.json().get('message', 'Erro desconhecido')}")
        return None
...
def upload_file_to_github(token, repo_full_name, file_path):
    ...
    print(f"[GITHUB] Tentando fazer upload de {os.path.basename(file_path)}")
    ...
    data = {
        "message": f"Upload de dados de backtest: {os.path.basename(file_path)}",
    ...
    if response.status_code in [200, 201]:
        print("[GITHUB] Upload concluído com sucesso.")
        return response.json()['content']['html_url']
    else:
        print(f"[ERRO GITHUB] Falha ao fazer upload ({response.status_code}): {response.json().get('message', 'Erro desconhecido')}")
        return None
        ...
        try:
            ...
            if repo_full_name:
                ...
                if upload_url:
                    ...
                    print(f"  AVISO: Por favor, EXCLUA o repositório '{repo_full_name}' após o download.")
                    print(f"{'*'*80}\n")
                else:
                    print("[ERRO FATAL] Falha no upload para o GitHub.")
            else:
                print("[ERRO FATAL] Falha na criação do repositório GitHub.")
