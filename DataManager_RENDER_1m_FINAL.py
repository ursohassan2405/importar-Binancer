#!/usr/bin/env python3
"""
DATA MANAGER RENDER 1M - Otimizado para Download Rápido, Cronometragem e Upload Único
"""
...
def upload_zip_to_temp_service(file_path):
    """Faz o upload do arquivo para transfer.sh e retorna o link."""
    print(f"\n{'='*70}")
    print(f"GERANDO LINK DE DOWNLOAD ÚNICO")
    print(f"{'='*70}")
    
    try:
        # Comando para upload para transfer.sh
        command = f"curl --upload-file {file_path} https://transfer.sh/{os.path.basename(file_path)}"
        
        # Executa o comando e captura a saída
        result = subprocess.run(command, shell=True, capture_output=True, text=True, check=True)
        
        upload_link = result.stdout.strip()
        
        print(f"[SUCESSO] Arquivo ZIP enviado.")
        return upload_link
        
    except subprocess.CalledProcessError as e:
        print(f"[ERRO UPLOAD] Falha ao enviar o arquivo: {e.stderr.strip()}")
        return "ERRO_UPLOAD"
    except FileNotFoundError:
        print(f"[ERRO UPLOAD] Comando 'curl' não encontrado.")
        return "ERRO_CURL"
    finally:
        print(f"{'='*70}\n")
...
def main():
    ...
    # --- Etapa Final: Compactar e Upload ---
    if files_to_zip:
        try:
            ...
            # Faz o upload e imprime o link
            download_link = upload_zip_to_temp_service(ZIP_PATH)
            
            print(f"\n\n{'*'*80}")
            print(f"LINK DE DOWNLOAD ÚNICO (VÁLIDO POR 24 HORAS):")
