# DATA MANAGER INSTITUCIONAL ELITE V16 - V16 SILENT EDITION
# 1. ENDPOINTS 100% SPOT (api.binance.com)
# 2. SILENT MODE: Logs reduzidos para máxima velocidade no Render
...
def log(msg, force=False):
    """Imprime apenas se for forçado ou em intervalos longos."""
    if force:
        print(msg)
        sys.stdout.flush()
...
def download_agg_trades_sequential(symbol, start_ms, end_ms, min_val_usd=500):
    log(f"[INFO] Iniciando Captura SPOT Silenciosa (Filtro: >${min_val_usd})", force=True)
    ...
    with requests.Session() as session:
        while last_ts < end_ms:
            try:
                ...
                # Log apenas a cada 100.000 baleias para não travar o Render
                if total_whales > 0 and total_whales % 100000 < 1000:
                    dt = pd.to_datetime(last_ts, unit='ms').strftime('%Y-%m-%d %H:%M')
                    log(f"    >>> Progresso: {dt} | Baleias: {total_whales}", force=True)
                
                if last_ts >= end_ms: break
                time.sleep(0.01) # Delay reduzido para máxima velocidade
    ...
    log(f"[SUCESSO] Total de {total_whales} baleias capturadas.", force=True)
    return np.vstack(all_chunks)

def download_klines_parallel(symbol, start_ms, end_ms):
    log("[INFO] Baixando Klines SPOT...", force=True)
...
def processar_final(rows_1m, trades_np, tf_str):
    log(f"[INFO] Consolidando para {tf_str}...", force=True)
...
def upload_to_catbox(file_path):
    log("[UPLOAD] Enviando para Catbox...", force=True)
    ...
    try:
        ...
        if res.status_code == 200:
            log("\n" + "="*60, force=True)
            log("   LINK DE DOWNLOAD DIRETO:", force=True)
            log(f"   {res.text}", force=True)
            log("="*60 + "\n", force=True)
    except: pass
