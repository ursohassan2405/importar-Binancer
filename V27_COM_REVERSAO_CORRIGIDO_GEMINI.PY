...
def checar_dataframe_tf(df_tf, tf_label):
    # 1. Padronizar o nome para 'timestamp' caso esteja como 'ts'
    if "ts" in df_tf.columns and "timestamp" not in df_tf.columns:
        df_tf = df_tf.rename(columns={"ts": "timestamp"})
    
    if "timestamp" not in df_tf.columns:
        raise ValueError(f"Dataset TF {tf_label} não contém 'timestamp' nem 'ts'. Colunas: {df_tf.columns.tolist()}")

    # 2. Garantir que seja numérico (int64) para o merge_asof não se perder
    df_tf["timestamp"] = pd.to_numeric(df_tf["timestamp"])

    # 3. Ordenação rigorosa (Obrigatório para merge_asof)
...
# =========================================================================
# 🔧 CORREÇÃO FINAL: BLINDAGEM CONTRA NaN NO RENDER (Substitua o seu bloco por este)
# =========================================================================

# 1. Identificar colunas críticas
colunas_pivots_abs = [c for c in df_all.columns if any(x in c.lower() for x in ['pivot', 'absorcao', 'absorção'])]
colunas_contexto = [c for c in df_all.columns if 'ctx_' in c]

# 2. Diagnóstico pré-correção
print(f"\n[DIAG] ANTES da blindagem: {len(df_all)} linhas")
print(f"[DIAG] NaNs em Pivots/Absorção: {df_all[colunas_pivots_abs].isna().sum().sum()}")
print(f"[DIAG] NaNs em Contextos (Multi-TF): {df_all[colunas_contexto].isna().sum().sum()}")

# 3. Aplicar preenchimento em Pivots/Absorção (Zerar o que é nulo)
if colunas_pivots_abs:
    df_all[colunas_pivots_abs] = df_all[colunas_pivots_abs].fillna(0)

# 4. APLICAR BLINDAGEM NOS CONTEXTOS (O Segredo para o Render)
# Em vez de apagar, o ffill/bfill "estica" o dado de 1d para cobrir os buracos de sincronia
if colunas_contexto:
    print(f"[DIAG] Aplicando ffill/bfill em {len(colunas_contexto)} colunas ctx...")
    df_all[colunas_contexto] = df_all[colunas_contexto].ffill().bfill()

# 5. Limpeza final de segurança
df_all = df_all.replace([np.inf, -np.inf], np.nan)
df_all = df_all.dropna()

print(f"[DIAG] APÓS blindagem e dropna: {len(df_all)} linhas")
print("✅ SUCESSO: Dataset pronto para treino institucional.")
# =============================================================================
