...
def treinar_k(df, k, X_cols):
    ...
    # Criar target - REMOVE NaNs no final
    close_fut = df['close'].shift(-k)
    mask_valid = close_fut.notna()  # ← FIX: Remove NaNs
    
    y = (close_fut > df['close']).astype(int).values
    X = df[X_cols].values
    
    # Aplicar máscara para remover NaNs
    X = X[mask_valid]
    y = y[mask_valid]
    
    # Validação: target deve ter variação
    if len(np.unique(y)) < 2:
        print(f"K{k}:SKIP(degenerado)", end=" ", flush=True)
        return None, None, 0
