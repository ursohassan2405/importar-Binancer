# DATA MANAGER INSTITUCIONAL ELITE V16 - V15 SPOT EDITION
# 1. ENDPOINTS 100% SPOT (api.binance.com) - IGUAL AO LOCAL V33
# 2. ID Discovery Inquebrável (Captura Sequencial por ID)
# 3. Persistência de Conexão (Auto-Retry 10x)
# 4. Sincronização Forçada de Timestamps (Merge Garantido)
...
def get_first_id_by_time(symbol, start_ms):
    """Descobre o primeiro ID de trade para o tempo inicial no SPOT."""
...
def download_agg_trades_sequential(symbol, start_ms, end_ms, min_val_usd=500):
    """Captura 100% dos trades SPOT usando ID Discovery e progressão sequencial."""
    log(f"\n[INFO] Iniciando Captura de Baleias SPOT (Filtro: >${min_val_usd})")
    
    curr_id = get_first_id_by_time(symbol, start_ms)
    if not curr_id:
        log("[ERRO] Não foi possível descobrir o ID inicial no SPOT. Abortando.")
        return None
    ...
    with requests.Session() as session:
        while last_ts < end_ms:
            try:
                ...
                if curr_id % 10000 < 1000:
                    dt = pd.to_datetime(last_ts, unit='ms').strftime('%Y-%m-%d %H:%M')
                    log(f"    >>> Progresso SPOT: {dt} | Baleias: {total_whales}")
                
                if last_ts >= end_ms: break
                time.sleep(0.05)
                
            except Exception as e:
                log(f"\n[!] Erro de conexão no SPOT: {e}. Retentando em 5s...")
                time.sleep(5)
                continue
                
    if not all_chunks: return None
    log(f"[INFO] Total de {total_whales} trades institucionais SPOT capturados.")
    return np.vstack(all_chunks)

def download_klines_parallel(symbol, start_ms, end_ms):
    log(f"\n[INFO] Baixando Klines SPOT de 1m...")
...
def processar_final(rows_1m, trades_np, tf_str):
    log(f"\n[INFO] Consolidando SPOT para {tf_str}...")
