# DATA MANAGER INSTITUCIONAL ELITE V16 - V6 ULTIMATE RESILIENT
# 1. Corrige Diluição (Sliding Window)
# 2. Captura Total (ID Discovery + Time Fallback)
# 3. Log a cada 10.000 trades
# 4. Upload Multi-Rota (GoFile + File.io)
# 5. Auto-Header Fix (Evita KeyError 'close')
...
def download_agg_trades_parallel(symbol, start_ms, end_ms, min_val_usd=500):
    ...
    use_id = True
    if curr_id is None:
        print("[AVISO] ID inicial não encontrado. Usando fallback por tempo...")
        use_id = False
        curr_ts = start_ms
        ...
        while True:
            try:
                if use_id:
                    ...
                    url = f"https://fapi.binance.com/fapi/v1/aggTrades?symbol={symbol}&startTime={curr_ts}&limit=1000"
...
def upload_multi_route(file_path):
    print(f"\n[UPLOAD] Tentando Rota 1 (GoFile)...")
    try:
        server = requests.get("https://api.gofile.io/getServer").json()['data']['server']
        res = requests.post(f"https://{server}.gofile.io/uploadFile", files={'file': open(file_path, 'rb')})
        if res.status_code == 200:
            print(f"   LINK GOFILE: {res.json()['data']['downloadPage']}")
            return
    except: print("   Rota 1 falhou.")

    print(f"[UPLOAD] Tentando Rota 2 (File.io)...")
    try:
        res = requests.post("https://file.io", files={'file': open(file_path, 'rb')})
        if res.status_code == 200:
            print(f"   LINK FILE.IO: {res.json()['link']}")
            return
    except: print("   Rota 2 falhou.")
...
def main():
    ...
    print(f"\n[SUCESSO] Arquivo: {zip_name}")
    upload_multi_route(zip_name)
