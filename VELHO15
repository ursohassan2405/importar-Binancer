# DATA MANAGER INSTITUCIONAL ELITE V16 - V4 TURBO CLEAN (FULL VERSION)
# 1. Corrige Diluição (Sliding Window)
# 2. Captura Total (ID Discovery)
# 3. Log a cada 10.000 trades
# 4. Upload para GoFile (Link no Log)
...
def download_agg_trades_parallel(symbol, start_ms, end_ms, min_val_usd=500):
    ...
    if first_id is None:
        print("[ERRO] ID inicial não encontrado.")
        return None
        ...
        while True:
            ...
            try:
                ...
                if res.status_code == 429:
                    print("[RATE LIMIT] Aguardando 20s...")
                    time.sleep(20)
                    continue
                ...
                # LOG AJUSTADO PARA 10.000 TRADES (COMO VOCÊ PEDIU)
                if last_id_seen % 10000 < 1000:
                    dt = pd.to_datetime(last_ts, unit='ms').strftime('%Y-%m-%d %H:%M')
                    print(f"    >>> Progresso: {dt} | Baleias: {total_whales} | ID: {last_id_seen}")
                
                if last_ts > end_ms:
                    print(f"[OK] Data final atingida: {pd.to_datetime(last_ts, unit='ms')}")
                    break
                ...
                print(f"[ERRO] Falha na conexão: {e}. Retentando em 5s...")
                time.sleep(5)
...
def download_futures_klines(symbol, interval, start_ms, end_ms):
    print(f"\n[INFO] Baixando Klines de 1m para Agregação...")
...
def processar_microestrutura_dinamica(df_klines_1m, trades_np, tf_str):
    print(f"\n[INFO] Aplicando Sliding Window e Consolidando para {tf_str}...")
...
def upload_to_gofile(file_path):
    print(f"\n[GOFILE] Iniciando upload para servidor seguro...")
    try:
        ...
        if res.status_code == 200:
            link = res.json()['data']['downloadPage']
            print("\n" + "="*60 + f"\n   LINK DE DOWNLOAD: {link}\n" + "="*60 + "\n")
    except: print("[ERRO] Falha no upload.")
...
def main():
    ...
    print(f"\n[SUCESSO] Arquivo gerado: {zip_name}")
    upload_to_gofile(zip_name)
