...
def upload_to_catbox(file_path):
    ...
    try:
        ...
        if res.status_code == 200:
            print("\n" + "="*60)
            print("   LINK DE DOWNLOAD DIRETO:")
            print(f"   {res.text}")
            print("="*60 + "\n")
        else:
            print(f"[ERRO UPLOAD] Status: {res.status_code}")
